...
function hardcodedAI_lvl_05(_game, _playerSelf, _playerOther) constructor
{
	game = _game
	...
	function SetOutputsToAchieveTargetVelocity(_targetDirection, _maxVelocity)
	{
		...
		var directionsMatch = ((180 - (abs(angle_difference(currentDirection, _targetDirection))+90)) /90 + 1)/1
		...
		{
			xoutput += -lengthdir_x(velocity - _maxVelocity, currentDirection)
			youtput += -lengthdir_y(velocity - _maxVelocity, currentDirection)
		}
	}
	...
	function OutOfBoundsEmergencyBreak()
	{
		...
		var fakeradius = game.game_radius - 5
