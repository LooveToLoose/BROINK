// Yup, number 7 is the unbeatable AI. It is actually beatable, but it's definitely quite hard.


function hardcodedAI_lvl_07(_game, _playerSelf, _playerOther) constructor
{
	game = _game
	...
	function StartOrReset()
	{
		xoutput = 0
		youtput = 0
		opening_y = (1-random(2))*0.5
		current_mode = mode_opening
		position_score = 0
		playerSelf.xinput = 0
		playerSelf.yinput = 0
	}
	...
	function Step()
	{
		...
		if selfDirectionAligned < 0.9 
		or otherDirectionAligned < 0.9 
	...
	function ModeOpening()
	{
		...
		if (game.barrier_left > barrier_duration-33 + abs(sin(opening_dir/180*pi)) * 0)
		{
	...
	function ModeDefensive()
	{
		...
		else
		{
			...
			mytargetdir = my_direction_from_center + (90 + own_speed*4 + outwards_percentage*30) * sign(angle_diff)
	...
	function SetOutputsToAchieveTargetVelocity(_targetDirection, _maxVelocity)
	{
		...
		var divider = 2
		var directionsMatch = ((180 - (abs(angle_difference(currentDirection, _targetDirection))+90)) /90 + 1)/divider
	...
	function SetOutputsToAchieveTargetVelocityOffensive(_targetDirection, _maxVelocity)
	{
		xoutput = lengthdir_x(100, _targetDirection)
		youtput = lengthdir_y(100, _targetDirection)
		var currentDirection = point_direction(0, 0, playerSelf.xspeed, playerSelf.yspeed)
		var velocity = point_distance(playerSelf.xspeed, playerSelf.yspeed, 0, 0)
		if velocity > _maxVelocity
		{
	...
	function OutOfBoundsEmergencyBreak()
	{
		...
		var fakeradius = game.game_radius - 40
